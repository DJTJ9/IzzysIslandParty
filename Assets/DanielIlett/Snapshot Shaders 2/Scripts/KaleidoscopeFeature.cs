...
{
    [DisallowMultipleRendererFeature("Snapshot Shaders 2/Kaleidoscope")]
    public class KaleidoscopeFeature : ScriptableRendererFeature
    {
    ...
    sealed class KaleidoscopeRenderPass : SnapshotRenderPass
    {
        protected override string ShaderName
        {
            get { return "Hidden/SnapshotShaders2/Kaleidoscope"; }
        }

        public KaleidoscopeRenderPass()
        {
            profilingSampler = new ProfilingSampler("SS2 - Kaleidoscope");
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            var maskHandleSettings = DrawMaskIfRequired<KaleidoscopeFeature>(renderGraph, frameData, settings, "SS2 Kaleidoscope Local Mask");
            ...
            using (var builder = renderGraph.AddRasterRenderPass<CopyPassData>("SS2 Kaleidoscope Copy Color", out var passData, profilingSampler))
            {
            ...
            using (var builder = renderGraph.AddRasterRenderPass<BasicPassData>("SS2 Kaleidoscope Main Pass", out var passData, profilingSampler))
            {
                ...
                builder.SetRenderFunc(static (BasicPassData data, RasterGraphContext context) =>
                {
                    ...
                    var settings = VolumeManager.instance.stack.GetComponent<KaleidoscopeSettings>();
                    data.material.SetFloat("_Segments", settings.segments.value);
