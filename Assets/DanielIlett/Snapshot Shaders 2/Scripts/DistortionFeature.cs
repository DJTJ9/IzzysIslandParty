...
{
    [DisallowMultipleRendererFeature("Snapshot Shaders 2/Distortion")]
    public class DistortionFeature : ScriptableRendererFeature
    {
        DistortionRenderPass pass;
        ...
        {
            var settings = VolumeManager.instance.stack.GetComponent<DistortionSettings>();
    ...
    sealed class DistortionRenderPass : SnapshotRenderPass
    {
        protected override string ShaderName
        {
            get { return "Hidden/SnapshotShaders2/Distortion"; }
        }

        public DistortionRenderPass()
        {
            profilingSampler = new ProfilingSampler("SS2 - Distortion");
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            var maskHandleSettings = DrawMaskIfRequired<DistortionFeature>(renderGraph, frameData, settings, "SS2 Distortion Local Mask");
            ...
            using (var builder = renderGraph.AddRasterRenderPass<CopyPassData>("SS2 Distortion Copy Color", out var passData, profilingSampler))
            {
            ...
            using (var builder = renderGraph.AddRasterRenderPass<BasicPassData>("SS2 Distortion Main Pass", out var passData, profilingSampler))
            {
                ...
                {
                    // Set Distortion effect properties.
                    var settings = VolumeManager.instance.stack.GetComponent<DistortionSettings>();
                    data.material.SetFloat("_Strength", settings.strength.value);
                    data.material.SetFloat("_Smoothing", settings.smoothing.value);
