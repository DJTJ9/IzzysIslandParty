...
namespace DanielIlett.SnapshotShaders2.URP
{
    ...
    sealed class BlurRenderPass : SnapshotRenderPass
    {
        ...
        // Keeping an array active rather than allocating each frame avoids GC alloc.
        private TextureHandle[] lightStreakMips = new TextureHandle[1];
        private string[] lightStreakMipNames = new string[] { "LightStreakMip0" };
            ...
            TextureHandle horizontalTexture, TextureHandle localMaskTextureHandle, bool useGlobalMask, bool useLocalMask, int passIndex)
        {
            ...
            using (var builder = renderGraph.AddRasterRenderPass<VerticalPassData>("SS2 Vertical Blur Pass", out var passData, profilingSampler))
            {
                ...
                builder.SetRenderAttachment(targetTexture, 0, AccessFlags.Write);
                builder.SetRenderFunc(static (VerticalPassData data, RasterGraphContext context) =>
                {
        ...
        private class LightStreaksPassData
        {
            ...
            public TextureHandle[] lightStreakMips;
            public int mipCount;
        }
            ...
            TextureHandle inputTexture, TextureHandle targetTexture, TextureHandle localMaskTextureHandle, bool useGlobalMask, bool useLocalMask)
        {
            ...
            if(lightStreakMips.Length < mipCount)
            {
                lightStreakMips = new TextureHandle[mipCount];
                lightStreakMipNames = new string[mipCount];

                for(int i = 0; i < mipCount; ++i)
                {
                    lightStreakMipNames[i] = $"LightStreakMip{i}";
                }
            }
            ...
            using (var builder = renderGraph.AddUnsafePass<LightStreaksPassData>("SS2 Light Streaks Pass", out var passData, profilingSampler))
            {
                ...
                passData.lightStreakMips = lightStreakMips;
                ...
                builder.SetRenderFunc(static (LightStreaksPassData data, UnsafeGraphContext context) =>
                {
                    ...
                    Blitter.BlitCameraTexture(cmd, data.inputTexture, data.lightStreakMips[0], data.material, 5);
                    ...
                    for (int i = 0; i < data.mipCount - 1; ++i)
                    {
                        ...
                        Blitter.BlitCameraTexture(cmd, data.lightStreakMips[src], data.lightStreakMips[dst], data.material, 6);
                    }

                    // Upsample and combine passes.
                    for (int i = data.mipCount - 1; i > 0; --i)
                    {
                        int src = i;
                        int dst = i - 1;

                        Blitter.BlitCameraTexture(cmd, data.lightStreakMips[src], data.lightStreakMips[dst], data.material, 7);
                    }

                    // Final combine pass.
                    Blitter.BlitCameraTexture(cmd, data.lightStreakMips[0], data.targetTexture, data.material, 7);
                });
            }
        }
