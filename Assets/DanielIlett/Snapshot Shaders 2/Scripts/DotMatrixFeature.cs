...
{
    [DisallowMultipleRendererFeature("Snapshot Shaders 2/Dot Matrix")]
    public class DotMatrixFeature : ScriptableRendererFeature
    {
        DotMatrixRenderPass pass;

        public override void Create()
        {
            pass = new DotMatrixRenderPass();
            name = "DotMatrix";
        }

        public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
        {
            var settings = VolumeManager.instance.stack.GetComponent<DotMatrixSettings>();
    ...
    sealed class DotMatrixRenderPass : SnapshotRenderPass
    {
        protected override string ShaderName
        {
            get { return "Hidden/SnapshotShaders2/DotMatrix"; }
        }

        public DotMatrixRenderPass()
        {
            profilingSampler = new ProfilingSampler("SS2 - DotMatrix");
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            var maskHandleSettings = DrawMaskIfRequired<DotMatrixFeature>(renderGraph, frameData, settings, "SS2 Dot Matrix Local Mask");
            ...
            using (var builder = renderGraph.AddRasterRenderPass<CopyPassData>("SS2 Dot Matrix Copy Color", out var passData, profilingSampler))
            {
            ...
            using (var builder = renderGraph.AddRasterRenderPass<BasicPassData>("SS2 Dot Matrix Main Pass", out var passData, profilingSampler))
            {
                ...
                {
                    // Set DotMatrix effect properties.
                    var settings = VolumeManager.instance.stack.GetComponent<DotMatrixSettings>();
                    data.material.SetInteger("_GapWidth", settings.gapWidth.value);
                    data.material.SetInteger("_DotSize", settings.dotSize.value);
