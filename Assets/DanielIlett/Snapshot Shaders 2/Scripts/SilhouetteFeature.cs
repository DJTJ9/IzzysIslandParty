...
{
    [DisallowMultipleRendererFeature("Snapshot Shaders 2/Silhouette")]
    ...
    sealed class SilhouetteRenderPass : SnapshotRenderPass
    {
        protected override string ShaderName
        {
            get { return "Hidden/SnapshotShaders2/Silhouette"; }
        }

        public SilhouetteRenderPass()
        {
            profilingSampler = new ProfilingSampler("SS2 - Silhouette");
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            var maskHandleSettings = DrawMaskIfRequired<SilhouetteFeature>(renderGraph, frameData, settings, "SS2 Silhouette Local Mask");
            ...
            using (var builder = renderGraph.AddRasterRenderPass<CopyPassData>("SS2 Silhouette Copy Color", out var passData, profilingSampler))
            {
            ...
            using (var builder = renderGraph.AddRasterRenderPass<BasicPassData>("SS2 Silhouette Main Pass", out var passData, profilingSampler))
            {
                ...
                builder.SetRenderFunc(static (BasicPassData data, RasterGraphContext context) =>
                {
                    ...
                    if(settings.useTextureRamp.value)
                    {
                        data.material.EnableKeyword("_USE_TEXTURE_RAMP");
                        data.material.SetTexture("_RampTexture", settings.textureRamp.value);
                    }
                    else
                    {
                        data.material.DisableKeyword("_USE_TEXTURE_RAMP");
                        data.material.SetColor("_NearColor", settings.nearColor.value);
                        data.material.SetColor("_FarColor", settings.farColor.value);
                    }
