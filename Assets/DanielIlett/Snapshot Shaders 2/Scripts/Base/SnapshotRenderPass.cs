...
namespace DanielIlett.SnapshotShaders2.URP
{
    public abstract class SnapshotRenderPass : ScriptableRenderPass
    {
        ...
        // Each pass keeps track of a tiny array of shader tags to avoid GC allocations.
        protected ShaderTagId[] shaderTagIDs = new ShaderTagId[2];
        ...
        // Each pass holds an array of shader tags which gets filled each frame rather than
        // creating a List dynamically to avoid GC alloc every frame.
        protected static ShaderTagId[] GetShaderTags(List<LightModeType> lightModeTypes, ref ShaderTagId[] shaderTagIDs)
        {
            if(shaderTagIDs.Length < lightModeTypes.Count)
            {
                shaderTagIDs = new ShaderTagId[lightModeTypes.Count];
            }

            for (int i = 0; i < lightModeTypes.Count; ++i)
            {
                shaderTagIDs[i] = new ShaderTagId(lightModeTypes[i].Convert());
            }
        ...
        protected RendererListHandle GetRendererList(RenderGraph renderGraph, UniversalRenderingData renderingData, Camera camera, MaskSettings settings, ShaderTagId[] shaderTagIDs, Material material)
        {
        ...
        protected static void DrawMaskObjects(MaskPassData data, RasterGraphContext context, bool clear)
        {
            if (clear)
            {
            ...
            {
                context.cmd.DrawMesh(data.skyboxMesh, data.transformationMatrix, data.maskMaterial);
            }
        }
            ...
            MaskSettings maskSettings, string textureName, string passName)
        {
            ...
            using (var builder = renderGraph.AddRasterRenderPass<MaskPassData>(passName, out var passData, profilingSampler))
            {
                ...
                passData.rendererList = GetRendererList(renderGraph, renderingData, camera, maskSettings, GetShaderTags(lightModes, ref shaderTagIDs), MaskMaterial);
