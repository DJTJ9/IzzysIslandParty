        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            using (var builder = renderGraph.AddRasterRenderPass<BasicPassData>("SS2 Filmic Main Pass", out var passData, profilingSampler))
            {
                ...
                builder.SetRenderFunc(static (BasicPassData data, RasterGraphContext context) =>
                {
                    ...
                    data.material.SetFloat("_NoiseThreshold", settings.noiseThreshold.value);

                    if(settings.noiseMode.value == NoiseDisplayMode.Modulate)
                    {
                        data.material.EnableKeyword("_NOISE_MODULATE");
                        data.material.DisableKeyword("_NOISE_OVERLAY");
                        data.material.DisableKeyword("_NOISE_OFF");
                    }
                    else if(settings.noiseMode.value == NoiseDisplayMode.Overlay)
                    {
                        data.material.DisableKeyword("_NOISE_MODULATE");
                        data.material.EnableKeyword("_NOISE_OVERLAY");
                        data.material.DisableKeyword("_NOISE_OFF");
                    }
                    else
                    {
                        data.material.DisableKeyword("_NOISE_MODULATE");
                        data.material.DisableKeyword("_NOISE_OVERLAY");
                        data.material.EnableKeyword("_NOISE_OFF");
                    }
