...
{
    [DisallowMultipleRendererFeature("Snapshot Shaders 2/Colorize")]
    public class ColorizeFeature : ScriptableRendererFeature
    {
    ...
    sealed class ColorizeRenderPass : SnapshotRenderPass
    {
        protected override string ShaderName
        {
            get { return "Hidden/SnapshotShaders2/Colorize"; }
        }

        public ColorizeRenderPass()
        {
            profilingSampler = new ProfilingSampler("SS2 - Colorize");
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            var maskHandleSettings = DrawMaskIfRequired<ColorizeFeature>(renderGraph, frameData, settings, "SS2 Colorize Local Mask");
            ...
            using (var builder = renderGraph.AddRasterRenderPass<CopyPassData>("SS2 Colorize Copy Color", out var passData, profilingSampler))
            {
            ...
            using (var builder = renderGraph.AddRasterRenderPass<BasicPassData>("SS2 Colorize Main Pass", out var passData, profilingSampler))
            {
                ...
                builder.SetRenderFunc(static (BasicPassData data, RasterGraphContext context) =>
                {
                    ...
                    var settings = VolumeManager.instance.stack.GetComponent<ColorizeSettings>();
                    data.material.SetColor("_TintColor", settings.tintColor.value);
