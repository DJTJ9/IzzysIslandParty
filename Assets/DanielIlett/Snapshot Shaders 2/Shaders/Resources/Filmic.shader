Shader "Hidden/SnapshotShaders2/Filmic"
{
	SubShader
    {
        ...
        Pass
        {
			...
			#pragma multi_compile_local_fragment _NOISE_MODULATE _NOISE_OVERLAY _NOISE_OFF
...
#if !defined(_NOISE_OFF)

				float mask = SampleMask(i.texcoord);

				float2 pos = i.texcoord * _ScreenParams.xy / _NoiseSize;

				float noise = perlin2D(pos) * _NoiseStrength;

#if defined(_NOISE_MODULATE)
				col.rgb = lerp(col.rgb, col.rgb - noise, mask);
#else
				noise = step(_NoiseThreshold, noise) * mask;
				col.rgb = lerp(col.rgb, 1.0f, noise);
#endif
#endif
