Shader "Hidden/SnapshotShaders2/Underwater"
{
	SubShader
    {
        ...
        Pass
        {
			...
			float2 _CausticsHeightThresholds;
            ...
            float4 frag (Varyings i) : SV_TARGET
            {
				...
				causticStrength += smoothstep(_CausticsHeightThresholds.x, _CausticsHeightThresholds.y, worldPos.y);
				underwaterCol.rgb += lerp(caustics, 0.0f, saturate(causticStrength));
		...
		Pass
        {
            ...
            float _CausticsEndFade;
            float _CausticsColorSeparation;
			float4 _CausticsHeightThresholds;
            ...
            float4 frag (Varyings i) : SV_TARGET
            {
				...
				float3 worldPos = ComputeWorldSpacePosition(waveUV, rawDepth, UNITY_MATRIX_I_VP);// + _Offset;
				float3 lightSpaceWorldPos = mul((half3x3)_MainLightMatrix, worldPos).xyz;

#if _USE_CAUSTICS_COLOR_SEPARATION
				float caustics1R = SAMPLE_TEXTURE2D(_CausticsTexture, sampler_LinearRepeat, (lightSpaceWorldPos + _CausticsScrollVelocity1 * _Time.y - _CausticsColorSeparation).xy * _CausticsTiling.x).r;
				float caustics1G = SAMPLE_TEXTURE2D(_CausticsTexture, sampler_LinearRepeat, (lightSpaceWorldPos + _CausticsScrollVelocity1 * _Time.y).xy * _CausticsTiling.x).g;
				float caustics1B = SAMPLE_TEXTURE2D(_CausticsTexture, sampler_LinearRepeat, (lightSpaceWorldPos + _CausticsScrollVelocity1 * _Time.y + _CausticsColorSeparation).xy * _CausticsTiling.x).b;

				float caustics2R = SAMPLE_TEXTURE2D(_CausticsTexture, sampler_LinearRepeat, (lightSpaceWorldPos + _CausticsScrollVelocity2 * _Time.y - _CausticsColorSeparation).xy * _CausticsTiling.y).r;
				float caustics2G = SAMPLE_TEXTURE2D(_CausticsTexture, sampler_LinearRepeat, (lightSpaceWorldPos + _CausticsScrollVelocity2 * _Time.y).xy * _CausticsTiling.y).g;
				float caustics2B = SAMPLE_TEXTURE2D(_CausticsTexture, sampler_LinearRepeat, (lightSpaceWorldPos + _CausticsScrollVelocity2 * _Time.y + _CausticsColorSeparation).xy * _CausticsTiling.y).b;

				float3 caustics1 = float3(caustics1R, caustics1G, caustics1B);
				float3 caustics2 = float3(caustics2R, caustics2G, caustics2B);
#else
				float3 caustics1 = SAMPLE_TEXTURE2D(_CausticsTexture, sampler_LinearRepeat, (lightSpaceWorldPos + _CausticsScrollVelocity1 * _Time.y).xy * _CausticsTiling.x).rgb;
				float3 caustics2 = SAMPLE_TEXTURE2D(_CausticsTexture, sampler_LinearRepeat, (lightSpaceWorldPos + _CausticsScrollVelocity2 * _Time.y).xy * _CausticsTiling.y).rgb;
#endif
